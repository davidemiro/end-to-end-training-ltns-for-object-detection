...
def disjunction_of_literals(literals,label="no_label"):
    ...
    literals_tensor = tf.concat(list_of_literal_tensors,0)
    #literals_tensor = tf.Print(literals_tensor, [tf.shape(literals_tensor)],'Literals_list'+label)
    ...
    if default_tnorm=="luk":
     #   literals_tensor = tf.Print(literals_tensor, [tf.shape(literals_tensor)],'l'+label)
        result = tf.minimum(1.0,tf.reduce_sum(literals_tensor,1, keep_dims=True))
      #  result = tf.Print(result, [tf.shape(result)],'luk'+label)
    ...
    if default_aggregator == "hmean":
       ...
       # h = tf.Print(h, [tf.shape(h)],'h'+label)
        return h
...
class Domain:
    def __init__(self,columns, dom_type="float",label=None):
        ...
        self.tensor = tf.Print(self.tensor,[self.tensor])
...
class Predicate:
    ...
    def tensor(self,domain=None):
        ...
        X = domain.tensor
        #X = tf.Print(X, [tf.shape(X),X.name])
...
class KnowledgeBase:
    ...
    def __init__(self,label,clauses,save_path="",params={}):
        #print "defining the knowledge base",label
        self.label = label
        self.clauses = clauses
        self.parameters = [par for cl in self.clauses for par in cl.parameters]
        default_layers = params['default_layers']
        default_smooth_factor = params['default_smooth_factor']
        default_tnorm = params['default_tnorm']
        default_optimizer = params['optimizer']
        default_aggregator = params['default_aggregator']
        default_positive_fact_penality = params['default_positive_fact_penality']
        default_clauses_aggregator = params['default_clauses_aggregator']
        if not self.clauses:
            self.tensor = tf.constant(1.0)
        else:
            clauses_value_tensor = tf.concat( [cl.tensor for cl in clauses],1)
            #clauses_value_tensor = tf.Print(clauses_value_tensor,[tf.shape(clauses_value_tensor)])
            ...
            if default_clauses_aggregator == "hmean":
                ...
                #self.tensor = tf.Print(self.tensor,[tf.shape(self.tensor)])
        ...
        #self.tensor = tf.Print(self.tensor,[tf.shape(clauses_value_tensor)])
