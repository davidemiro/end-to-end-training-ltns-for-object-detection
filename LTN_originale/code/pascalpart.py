...
pairs_of_objects = ltn.Domain(2*(number_of_features-1),label="a_pair_of_bounding_boxes")

isOfType = {}
for t in selected_types:
    isOfType[t] = ltn.Predicate("is_of_type_"+t.decode('UTF-8'),objects,layers=6)
isPartOf = ltn.Predicate("is_part_of",pairs_of_objects)

objects_of_type = {}
objects_of_type_not = {}
for t in selected_types:
    objects_of_type[t] = ltn.Domain(number_of_features-1,label="objects_of_type_"+t.decode('UTF-8'))
    objects_of_type_not[t] = ltn.Domain(number_of_features-1,label="objects_of_type_not_" + t.decode('UTF-8'))

object_pairs_in_partOf = ltn.Domain((number_of_features-1) * 2,
                                    label="object_pairs_in_partof_relation")
object_pairs_not_in_partOf = ltn.Domain((number_of_features-1) * 2,
...
def get_data_unofficial(train_or_test_swritch,max_rows=10000000):
    ...
    types_of_data = np.genfromtxt('label.csv',dtype='S12')
    pairs_of_data =np.genfromtxt('partOf.csv')
    partOf_of_pair_of_data =np.genfromtxt('label_partOf.csv')
        
    
    return data, pairs_of_data, types_of_data, partOf_of_pair_of_data
...
def get_part_whole_ontology():
    ...
    for t in selected_types:
        #t = t.decode('UTF-8')
        selected_parts_of_whole[t] = [p for p in parts_of_whole[t.decode('UTF-8')] if p in selected_types]
        selected_wholes_of_part[t] = [w for w in wholes_of_part[t.decode('UTF-8')] if w in selected_types]
    import pdb;pdb.set_trace()
