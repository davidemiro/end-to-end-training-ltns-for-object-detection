...
def get_map(pred, gt,w,h,f):
	...
	for k,m in mapping_id_pred.items():
		...
		for z,n in mapping_id_pred.items():
			...
			if k == n['partOf']:
				pairs_partOF_true.append(pair)
			else:
				pairs_partOF_false.append(pair)
	return T, P , pairs_partOF_true, pairs_partOF_false, data, label
...
for idx, img_data in enumerate(test_imgs):
	...
	pairs_partOF_true +=pt
	pairs_partOF_false +ptf
	data +=d
	label +=l
	for key in t.keys():
		if key not in T:
	...
	#print(P)
'''
np.savetxt('part_of_positive.csv',pairs_partOF_true)
np.savetxt('part_of_negative.csv',pairs_partOF_false)
np.savetxt('data.csv',data)
np.savetxt('label.csv',label,fmt='%s')
'''
